  ...
  return [
    {
      ...
      content: [
        ...
        "- Transaction semantics lock: when the outline/memory/world facts define a rule, bargain, cost, exchange, debt, curse, contract, or evidence trade, preserve its subject, object, beneficiary, loss target, and payer exactly. Do not reinterpret, transfer, upgrade, or protagonist-center it.",
        "- Transaction semantics lock: before planning a bargain-related chapter, identify what is pawned/lost, who loses it, what is received, who receives it, and who is affected. chapterGoal/plannedOutcome/mustHitConflicts must keep those roles stable.",
        "- Transaction semantics lock: if a rule says 'A loses B as first rescue route / trust route / memory route / permission route', the consequence is that B no longer reliably uses A in that role. Do NOT silently replace A with the protagonist or another character unless the outline explicitly says the rule transfers to that person.",
        "- Failed-workaround rule: the concrete cost must block an obvious workaround. If the same result can be obtained by changing a name, channel, wording, or tool, the plan must either specify why that workaround fails or choose a stronger consequence.",
        isEarlyChapter
          ? "- Pawnshop first-transaction staging (urban_rule_pawnshop_v1, chapter 1-3): keep the first case alive. Chapter 1 = client/request/candidate rule, no completed deal. Chapter 2 = hidden fault/desire + exact price, no signature, no transaction成立/完成, no post-completion price writing, no second client and no cancel/pause/redeem mechanics. Chapter 3 = lock/complete first exchange + first irreversible backlash, not full resolution."
          : undefined,
